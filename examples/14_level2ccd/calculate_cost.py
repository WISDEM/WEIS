...
def Calc_TC(wt_init, modeling_options, analysis_options,DV):
    ...
    # ballast volume
    #TM["components"]["floating_platform"]["members"][0]["internal_structure"]["ballasts"][0]["volume"] = BV
    ...
    return tcc_per_kW,bos_per_kW,MR,opex_per_kW,fcr,wlf,TM
...
BV_b = np.linspace(bv_bounds[0],bv_bounds[1],ns)

# meshgrid
CS,BV = np.meshgrid(CS_b,BV_b)

CS = CS.reshape([-1,1],order = 'F'); CS = CS.squeeze()
BV = BV.reshape([-1,1],order = 'F'); BV = BV.squeeze()

DV_ = np.zeros((ns*ns,2))
DV_[:,0] = CS; DV_[:,1] = BV
DV_ = np.array([[51.75,169.333]])
...
for i in range(ns*ns):
    ...
    tcckW,boskW,MR,opexkW,fcr,wlf,TM = Calc_TC(wt_init, modeling_options, analysis_options,DV_[i,:])
