...
def generate_test_data(this_dir,modeling_options,wt_init,dlc_generator):
...
if __name__ == '__main__':
    ...
    if MPI:
        ...
        n_DV = 1 # fix number of design variables as 1
        max_cores = MPI.COMM_WORLD.Get_size()

        # calculate number of OF runs
        max_parallel_OF_runs = max([int(np.floor((max_cores - n_DV) / n_DV)), 1])
        n_OF_runs_parallel = min([int(n_OF_runs), max_parallel_OF_runs])

        # get different communicators and color map
        comm_map_down, comm_map_up, color_map = map_comm_heirarchical(n_DV, n_OF_runs_parallel, openmp=False)
        ...
        if rank < len(color_map):
            try:
                ...
                raise ValueError('The number of finite differencing variables is {} and the correct number of cores were not allocated'.format(n_DV))
        else:
    ...
    plot_path = 'plot_doe_result_2';save_flag = True

    if rank == 0 and not os.path.isdir(plot_path):
