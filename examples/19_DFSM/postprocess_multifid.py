...
if __name__ == '__main__':
    ...
    lf_warmstart_file = 'lf_ws_file_oz_36.dill'
    hf_warmstart_file = 'hf_ws_file_oz_36.dill'
    ...
    for i_ob,obj in enumerate(objs):
        ...
        cbar = ax1.tricontourf(DV[:,0],DV[:,1],outputs_hf)
        ax1.plot(DV[:,0],DV[:,1],'.k')
        #ax1.set_ylim([0.75,3.0])
        ...
        cbar = ax.tricontourf(DV[:,0],DV[:,1],outputs_lf)
        ax.plot(DV[:,0],DV[:,1],'.k')
