...
def extract_results(driver_cases,obj):
...
doe_output_dir = this_dir + os.sep + 'outputs' + os.sep + 'doe_mpi_5w_6s'
doe_sql_file = doe_output_dir + os.sep + 'log_opt.sql'

opt_output_dir = this_dir + os.sep + 'outputs' + os.sep + 'opt3_5w_6s'
opt_sql_file = opt_output_dir + os.sep + 'log_opt.sql'
...
# load sql file
cr_opt = om.CaseReader(opt_sql_file)
opt_driver_cases = cr_opt.get_cases('driver')

# load design variables
DEL,DV,n_iter,n_var = extract_results(doe_driver_cases,obj)
...
Obj,DV_opt,n_iter_opt,n_var = extract_results(opt_driver_cases,obj)

DV_opt = np.reshape(DV_opt,[n_iter_opt,n_var],order = 'C')

fig,ax = plt.subplots(1)

CP = ax.contourf(O,Z,DEL,30,cmap = plt.cm.viridis)
#ax.plot(0.2,1,'.',color = 'r',markersize = 12)
