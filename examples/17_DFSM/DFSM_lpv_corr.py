...
def perform_and(Iw1,Iw2):
    
    Iw = []
    
    for i in range(len(Iw1)):
        w1 = Iw1[i];w2 = Iw2[i]
        w = w1 and w2
...
def construct_lpv(SimulationDetails,nmodels = 30,offset = 3,buff = 1):
    
    # extract inputs and outputs
    controls,states,outputs,time = SimulationDetails.provide_inputs_outputs()
    
    ncontrols = len(controls.T)
    nstates = len(states.T)
    ...
    # extract wind speed info
    wmin = np.min(wind)+buff
    wmax = np.max(wind)-buff
    
    maxw = np.max(wind)
    minw = np.min(wind)
    
    # create wind speeds
    W_ = np.linspace(wmin,wmax,nmodels)
    IW = []

    A_ = []; B_ = []
    eig_ = []
    
    # loop through and evaluate linear models corresponding to wind speed
    for k in range(nmodels):
        ...
        # find all the values in the range

        Iw1 = (w+offset >= wind) 
        Iw2 = (w-offset <= wind)

        Iw = perform_and(Iw1,Iw2)
        ...
        AB = lstsq(inputs_,outputs_,rcond = None); AB = AB[0]
        
        # add
        AB = AB.T
        A = AB[0:nstates,0:nstates]
        A_.append(A)
        
        B = AB[:,nstates:ninputs]
        B_.append(B)
    ...
    A_interp = np.array(A_)
    B_interp = np.array(B_)
    
    A_pp = PchipInterpolator(W_,A_interp)
    A_fun = lambda w: A_pp(w)
    
    B_pp = PchipInterpolator(W_,B_interp)
    B_fun = lambda w: B_pp(w)
    
    return A_fun,B_fun,nstates,ncontrols,wmin,wmax
        
        
def test_lpv(A_fun,B_fun,inputs,nstates,ncontrols,wmin,wmax):
    
    nt = len(inputs)
    
    dx = np.zeros((nt,nstates))
    ...
    for i in range(nt):
        ...
        states = inputs[i,0:nstates]
        controls = inputs[i,nstates:nstates+ncontrols]
        
        A_ = A_fun(w);B_ = B_fun(w)
        
        dx[i,:] = np.dot(A_,states) + np.dot(B_,controls)
...
def odefun_linear_corr(t,x,u_fun,A_fun,B_fun,wmin,wmax,NN,scaler_input,scaler_output):
    ...
    if not(NN == None):
        ...
        dx = dx + scaler_output.inverse_transform(NN.predict(input_scaled)) # + NN.predict(input_scaled)
...
#------------------------------------------------------------------------------    
def run_comparisons(SimulationDetails,nt,A_fun,B_fun,SM,scaler_input,scaler_output,SM_type,wmin,wmax,save_flag):
    
     # reduce samples
     time,controls,states,state_derivatives,state_derivative2_ = reduce_samples(nt,SimulationDetails)
     
     ncontrols = len(controls.T);nstates = len(states.T)
     ...
     dx_linfit = test_lpv(A_fun,B_fun,inputs,nstates,ncontrols,wmin,wmax)
     
     # add corrective function
     dx_error = scaler_output.inverse_transform(SM.predict(inputs_scaled))

     dx_combined = np.copy(dx_linfit) + dx_error
...
#------------------------------------------------------------------------------     
def run_simulation(tspan,x0,u_fun,A_fun,B_fun,wmin,wmax,solve_options,SimulationDetails,scaler_input,scaler_output,t_eval,iteration,dx_ind,save_flag,corrective_function = None):
    ...
    if corrective_function:
        ...
        t1 = time1.time()
        sol =  solve_ivp(odefun_linear_corr,tspan,x0,method=solve_options['method'],args = (u_fun,A_fun,B_fun,wmin,wmax,corrective_function,scaler_input,scaler_output),rtol = solve_options['rtol'],atol = solve_options['atol'])
        t2 = time1.time()
        
        t_sim = t2-t1
        
        print('')
        print('Time to simulate DFSM: '+str(t_sim))
        print('')
        
        # extract solution
        T = sol['t']
        x_dfsm = sol['y']; nstates = len(x_dfsm)
        x_dfsm = x_dfsm.T
        U = u_fun(T)
        ncontrols = len(U.T) 
        ...
        sol_lin = solve_ivp(odefun_linear_corr,tspan,x0,method=solve_options['method'],t_eval = T,args = (u_fun,A_fun,B_fun,wmin,wmax,None,scaler_input,scaler_output),rtol = solve_options['rtol'],atol = solve_options['atol'])
        x_linfit = sol_lin['y'];x_linfit = x_linfit.T
        ...
        # evaluate error
        dx_error = scaler_output.inverse_transform(corrective_function.predict(inputs_scaled))
...
def train_corrective_function(ncluster,inputs_error,dx_error,fun_type = 'GPR'):
    ...
    return corr_fun,fun_type,scaler_input,scaler_output,t_kmeans,t_gpr
...
if __name__ == '__main__':
    ...
    reqd_channels = ['Wave1Elev','B1Surge','B1Pitch']
    ...
    training_samples = SimulationDetails(simulation_name = 'Simulation 1',
                                       ...
                                       controls_ind = control_ind,states_ind = state_ind,filter_flag = 1,add_dx2 = 0,tmin = 00,tmax = 200)
    ...
    for icluster in nclusters:
        ...
        A_fun,B_fun,nstates,ncontrols,wmin,wmax = construct_lpv(training_samples,nmodels = icluster,offset = 0.5,buff = 0.1)
        t2 = time1.time()
        t_lpv = t2-t1
        
        print('')
        print('LPV construction time: '+str(t_lpv))
        print('')
        
        dx_fit = test_lpv(A_fun,B_fun,inputs_fit,nstates,ncontrols,wmin,wmax)
        dx_val = test_lpv(A_fun,B_fun,inputs_val,nstates,ncontrols,wmin,wmax)
    ...
    fun_type = 'GPR'
    ncluster_ = [700] #700 for below rated
    t_kmeans = np.zeros(len(ncluster_))
    t_gpr = np.zeros(len(ncluster_))
    
    for idx,ncluster in enumerate(ncluster_):
        print('')
        print(ncluster)
        
        t_flag = True
        #t1 = time1.time()
        corr_fun,fun_type,scaler_input,scaler_output,tk,tg = train_corrective_function(ncluster,inputs_error,dx_error,fun_type)
        #t2 = time1.time()
        ...
        t1 = time1.time()
        nt_ = 500
        run_comparisons(training_samples,nt_,A_fun,B_fun,corr_fun,scaler_input,scaler_output,fun_type,wmin,wmax,save_flag )
        
        nt_ = 500
        run_comparisons(validation_samples,nt_,A_fun,B_fun,corr_fun,scaler_input,scaler_output,fun_type,wmin,wmax,save_flag)
        t2 = time1.time()
        
        t_eval = t2-t1
        
        print('')
        print('Time to evaluate dfsm for 10000 pts: '+str(t_eval))
        print('')
        ...
        time_ = None;dx_ind = []
        dx_error_fit2,inputs_fit2 = run_simulation(tspan_fit,X0_fit,u_fun_fit,A_fun,B_fun,wmin,wmax,solve_options,training_samples,scaler_input,scaler_output,time_,iteration,dx_ind,save_flag,corr_fun)
        dx_error_val2,inputs_val2 = run_simulation(tspan_val,X0_val,u_fun_val,A_fun,B_fun,wmin,wmax,solve_options,validation_samples,scaler_input,scaler_output,time_,iteration,dx_ind,save_flag,corr_fun)    
