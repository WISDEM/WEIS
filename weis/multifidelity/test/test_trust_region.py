...
class Test(unittest.TestCase):
    def test_optimization(self):
        ...
        trust_region = SimpleTrustRegion(
            model_low, model_high, bounds, disp=False, num_initial_points=6
        )
    ...
    def test_1d_constrained_optimization_lower(self):
        ...
        np.testing.assert_allclose(results["optimal_design"], 0.737148, atol=1e-3)
        np.testing.assert_allclose(results["outputs"]["con"], 0.295269, atol=1e-5)
    ...
    def test_1d_constrained_optimization_upper(self):
        ...
        results = trust_region.optimize(plot=False, num_iterations=20, num_basinhop_iterations=10)

        np.testing.assert_allclose(results["optimal_design"], 0.111889, atol=1e-3)
        np.testing.assert_allclose(results["outputs"]["con"], 0.000157, atol=1e-5)
