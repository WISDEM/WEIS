...
class DLCGenerator(object):
    ...
    openfast_input_map = {
        ...
        'wind_direction': [
            ("InflowWind","PropagationDir"),
            ("ElastoDyn", "NacYaw"),
            ("ServoDyn", "YawNeut"),
        ],

        'wave_direction': ("HydroDyn","WaveDir"),
        'probability': ("DLC","probability"),
...
def make_equal_length(option_dict,target_name):
    ...
    for key in option_dict:
        if len(option_dict[key]) == 1:
            ...
            if key == 'probability':
                option_dict['probability'] /= target_len
...
def wrap_180(angle):
    ...
    angle_wrapped = np.mod(angle_np + 180, 360) - 180

    # Back to not-numpy
    angle_wrapped = angle_wrapped.tolist()
