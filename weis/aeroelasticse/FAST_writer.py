...
class InputWriter_OpenFAST(object):
        ...
        for afi in range(int(self.fst_vt['AeroDyn15']['NumAFfiles'])):
            ...
            if self.fst_vt['AeroDyn15']['AFTabMod'] == 2:
                ...
                if self.fst_vt['AeroDyn15']['af_data'][afi][0]['Ctrl'] == self.fst_vt['AeroDyn15']['af_data'][afi][1]['Ctrl']: 
                    num_tab = 1  # assume that all Ctrl values of the aerodynamic control device are identical if the first two are -> no DAC device!
                else:
            ...
            for tab in range(num_tab): # For writing multiple tables (different Re or Ctrl values)
                ...
                # commented out to remove integer designation f.write('{:<22d}   {:<11} {:}'.format(int(self.fst_vt['AeroDyn15']['af_data'][afi][tab]['Ctrl']), 'Ctrl', '! Control setting (must be 0 for current AirfoilInfo)\n')) 
    ...
    def write_OLAF(self):
        ...
        f.write('{:<22d} {:<11} {:}'.format(self.fst_vt['AeroDyn15']['OLAF']['RegDeterMethod'], 'RegDeterMethod','Method to determine the regularization parameters {0: Constant, 1: Optimized, 2: Chord-scaled, 3: dr-scaled, default: 0 }\n'))
