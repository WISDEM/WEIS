...
class FASTLoadCases(ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_output('max_pitch_rate',    val=0.0, units='deg/s', desc='Maximum pitch command rate')  # is this over a set of sims?
    ...
    def get_control_measures(self, sum_stats, chan_time, inputs, discrete_inputs, outputs, discrete_outputs):
        ...
        if self.options['modeling_options']['General']['openfast_configuration']['keep_time']:
            ...
            outputs['max_pitch_rate'] = max(max_pitch_rate)  / np.rad2deg(self.fst_vt['DISCON_in']['PC_MaxRat'])        # normalize by ROSCO pitch rate
        else:
