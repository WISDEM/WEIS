...
class WindPark(om.Group):
    ...
    def setup(self):
        ...
        # self.connect('sse.powercurve.rated_V',        'rlds_post.aero_rated.V_load')
        if modeling_options['Level1']['ServoSE']['flag']:
            if not (modeling_options['Level2']['flag'] == True or modeling_options['Level3']['flag'] == True):
                self.connect('sse.gust.V_gust',              ['rlds_post.aero_gust.V_load', 'rlds_post.aero_hub_loads.V_load'])
                self.connect('sse.powercurve.rated_Omega',   ['rlds_post.Omega_load', 'rlds_post.tot_loads_gust.aeroloads_Omega', 'rlds_post.constr.rated_Omega'])
                self.connect('sse.powercurve.rated_pitch',   ['rlds_post.pitch_load', 'rlds_post.tot_loads_gust.aeroloads_pitch'])
        ...
        if (modeling_options['Level2']['flag'] == True or modeling_options['Level3']['flag'] == True) and modeling_options['Level1']['ServoSE']['flag']:
            ...
            self.connect('sse.powercurve.rated_Omega',     'sse_tune.tune_rosco.rated_rotor_speed')
            ...
            self.connect('rlds.frame.flap_mode_freqs','sse_tune.tune_rosco.flap_freq', src_indices=[0])
            self.connect('rlds.frame.edge_mode_freqs','sse_tune.tune_rosco.edge_freq', src_indices=[0])
        ...
        elif (modeling_options['Level2']['flag'] == True or modeling_options['Level3']['flag'] == True) and modeling_options['Level1']['ServoSE']['flag']==False:
            exit("ERROR: WISDEM does not support openfast without the tuning of ROSCO")
        else:
            pass

        # Connections to rotor load analysis
        if (modeling_options['Level2']['flag'] == True or modeling_options['Level3']['flag'] == True) and modeling_options['openfast']['analysis_settings']['Analysis_Level'] == 2:
        ...
        self.connect('elastic.A',    'rlds_post.A')
        self.connect('elastic.EA',   'rlds_post.EA')
        self.connect('elastic.EIxx', 'rlds_post.EIxx')
        self.connect('elastic.EIyy', 'rlds_post.EIyy')
        self.connect('elastic.GJ',   'rlds_post.GJ')
        self.connect('elastic.rhoA', 'rlds_post.rhoA')
        self.connect('elastic.rhoJ', 'rlds_post.rhoJ')
        self.connect('elastic.x_ec', 'rlds_post.x_ec')
        self.connect('elastic.y_ec', 'rlds_post.y_ec')
        self.connect('elastic.precomp.xu_strain_spar', 'rlds_post.xu_strain_spar')
        self.connect('elastic.precomp.xl_strain_spar', 'rlds_post.xl_strain_spar')
        self.connect('elastic.precomp.yu_strain_spar', 'rlds_post.yu_strain_spar')
        self.connect('elastic.precomp.yl_strain_spar', 'rlds_post.yl_strain_spar')
        self.connect('elastic.precomp.xu_strain_te',   'rlds_post.xu_strain_te')
        self.connect('elastic.precomp.xl_strain_te',   'rlds_post.xl_strain_te')
        self.connect('elastic.precomp.yu_strain_te',   'rlds_post.yu_strain_te')
        self.connect('elastic.precomp.yl_strain_te',   'rlds_post.yl_strain_te')
        self.connect('blade.outer_shape_bem.s','rlds_post.constr.s')

        
        if (modeling_options['Level2']['flag'] == True or modeling_options['Level3']['flag'] == True) and modeling_options['openfast']['dlc_settings']['run_blade_fatigue']:
        ...
        if modeling_options['Level1']['DriveSE']['flag']:
            ...
            self.connect('sse.powercurve.rated_Omega', 'drivese_post.rated_rpm')
            self.connect('sse.powercurve.rated_Q',     'drivese_post.rated_torque')
            self.connect('control.rated_power',        'drivese_post.machine_rating')    
            ...
            self.connect('elastic.precomp.blade_mass',        'drivese_post.blade_mass')
            self.connect('elastic.precomp.mass_all_blades',   'drivese_post.blades_mass')
            self.connect('elastic.precomp.I_all_blades',      'drivese_post.blades_I')
            ...
            if modeling_options['flags']['generator']:
                ...
                if modeling_options['GeneratorSE']['type'] in ['pmsg_outer']:
                    ...
                    self.connect('sse.powercurve.rated_mech', 'drivese_post.generator.P_mech')
        ...
        if modeling_options['Level1']['DriveSE']['flag'] and modeling_options['Level1']['TowerSE']['flag']:
            ...
            if modeling_options['Level1']['ServoSE']['flag']:
                self.connect('sse.gust.V_gust',               'towerse_post.wind.Uref')
                
            self.connect('assembly.hub_height',           'towerse_post.wind_reference_height')  # TODO- environment
            self.connect('foundation.height',             'towerse_post.wind_z0') # TODO- environment
            ...
            self.connect('foundation.height',             'towerse_post.foundation_height')
            self.connect('tower.diameter',                'towerse_post.tower_outer_diameter_in')
            self.connect('tower.height',                  'towerse_post.tower_height')
            self.connect('tower.s',                       'towerse_post.tower_s')
        ...
        if (modeling_options['Level2']['flag'] == True or modeling_options['Level3']['flag'] == True):
            ...
            self.connect('drivese_post.hub_system_mass',                 'aeroelastic.hub_system_mass')
            self.connect('drivese_post.hub_system_I',                    'aeroelastic.hub_system_I')
            ...
            self.connect('drivese_post.rna_mass',          'aeroelastic.above_yaw_mass')
            self.connect('drivese_post.yaw_mass',                'aeroelastic.yaw_mass')
            self.connect('drivese_post.nacelle_I',               'aeroelastic.nacelle_I')
            self.connect('drivese_post.nacelle_cm',              'aeroelastic.nacelle_cm')
        ...
        if (modeling_options['Level2']['flag'] == True or modeling_options['Level3']['flag'] == True) and modeling_options['openfast']['dlc_settings']['run_power_curve'] and modeling_options['openfast']['analysis_settings']['Analysis_Level'] == 2:
            self.connect('aeroelastic.AEP',     'financese_post.turbine_aep')
        elif modeling_options['Level1']['ServoSE']['flag']:
            self.connect('sse.AEP',             'financese_post.turbine_aep')

        self.connect('tcc.turbine_cost_kW',     'financese_post.tcc_per_kW')
        if modeling_options['Level1']['BOS']['flag']:
        ...
        if modeling_options['flags']['control']:
            self.connect('control.rated_power',     'financese_post.machine_rating')
        ...
        if modeling_options['Level1']['ServoSE']:
            if modeling_options['Analysis_Flags']['OpenFAST'] and modeling_options['openfast']['dlc_settings']['run_power_curve'] and modeling_options['openfast']['analysis_settings']['Analysis_Level'] == 2:
                ...
                self.connect('sse.AEP',             'outputs_2_screen_weis.aep')
            self.connect('financese_post.lcoe',          'outputs_2_screen_weis.lcoe')
            
        self.connect('elastic.precomp.blade_mass',  'outputs_2_screen_weis.blade_mass')
