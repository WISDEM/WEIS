        ...
        if modeling_options['Level3']['flag']:
            ...
            if modeling_options["flags"]["tower"] and not modeling_options["flags"]["floating"]:
                ...
                if modeling_options["flags"]["tower"]:
                    self.connect('floatinse.tower.mass_den',                'aeroelastic.mass_den')
                    self.connect('floatinse.tower.foreaft_stff',            'aeroelastic.foreaft_stff')
                    self.connect('floatinse.tower.sideside_stff',           'aeroelastic.sideside_stff')
                    self.connect('floatinse.tower.sec_loc',                 'aeroelastic.sec_loc')
                    self.connect('floatinse.tower.section_height',    'aeroelastic.tower_section_height')
                    self.connect('floatinse.tower.outer_diameter',    'aeroelastic.tower_outer_diameter')
                    self.connect('floatinse.tower.z_param',                 'aeroelastic.tower_monopile_z')
                    self.connect('floatinse.tower_fore_aft_modes',     'aeroelastic.fore_aft_modes')
                    self.connect('floatinse.tower_side_side_modes',    'aeroelastic.side_side_modes')
                    self.connect('tower.cd',                        'aeroelastic.tower_cd')
                    self.connect('tower_grid.height',               'aeroelastic.tower_height')
                    self.connect('tower_grid.foundation_height',    'aeroelastic.tower_base_height')
                    self.connect('floatinse.tower.wall_thickness',    'aeroelastic.tower_wall_thickness')
                    self.connect('floatinse.tower.E',                       'aeroelastic.tower_E')
                    self.connect('floatinse.tower.G',                       'aeroelastic.tower_G')
                    self.connect('floatinse.tower.rho',                     'aeroelastic.tower_rho')
                    self.connect('monopile.transition_piece_mass',  'aeroelastic.transition_piece_mass')
