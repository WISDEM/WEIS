...
class WindPark(om.Group):
    ...
    def setup(self):
        ...
        dac_ivc.add_output('dac_ext',   val = np.ones(n_dac))
        dac_ivc.add_output('dac_start', val=np.zeros(n_dac),               desc='1D array of the start positions along blade span of the DAC device(s). Only values between 0 and 1 are meaningful.')
        dac_ivc.add_output('dac_end',   val=np.zeros(n_dac),               desc='1D array of the end positions along blade span of the DAC device(s). Only values between 0 and 1 are meaningful.')
        dac_ivc.add_output('chord_start',   val=np.zeros(n_dac),           desc='1D array of the positions along chord where the DAC device(s) start. Only values between 0 and 1 are meaningful.')
        dac_ivc.add_output('delta_max_pos', val=np.zeros(n_dac),           desc='1D array of the max value of the DAC devices.')
        dac_ivc.add_output('delta_max_neg', val=np.zeros(n_dac),           desc='1D array of the min value of the DAC devices.')
        ...
        tune_rosco_ivc.add_output('dac_kp_norm',      val=0.0,                    desc='DAC controller normalized gain') #TODO bem: may need to change naming convention
        tune_rosco_ivc.add_output('dac_tau',          val=0.0, units='s',         desc='DAC controller integral gain time constant')
        ...
        tune_rosco_ivc.add_output('Kp_flap',          val=0.0, units='s',         desc='Proportional term of the PI controller for the trailing-edge flaps') #TODO bem: may need to change naming convention
        ...
        if not modeling_options['Level3']['from_openfast']:
            ...
            self.connect('dac_ivc.dac_end',                   'xf.span_end')
            self.connect('dac_ivc.dac_ext',                   'xf.span_ext')
        ...
        if modeling_options['ROSCO']['flag']:
            ...
            if not modeling_options['Level3']['from_openfast']:     #from WISDEM models
                ...
                self.connect('xf.cl_interp_dac',               'sse_tune.airfoils_cl')
                self.connect('xf.cd_interp_dac',               'sse_tune.airfoils_cd')
                self.connect('xf.cm_interp_dac',               'sse_tune.airfoils_cm')
                ...
                self.connect('xf.dac_param',                    'sse_tune.airfoils_Ctrl')
                ...
                self.connect('rotorse.rs.frame.flap_mode_freqs','sse_tune.tune_rosco.flap_freq', src_indices=[0]) #TODO bem: may need to change naming convention
                ...
                self.connect('rosco_turbine.flap_freq',             'sse_tune.tune_rosco.flap_freq', src_indices=[0]) #TODO bem: may need to change naming convention
            ...
            if modeling_options['ROSCO']['DAC_Mode'] > 0:
                self.connect('tune_rosco_ivc.dac_kp_norm',    'sse_tune.tune_rosco.dac_kp_norm')
                self.connect('tune_rosco_ivc.dac_tau',     'sse_tune.tune_rosco.dac_tau')
        ...
        if modeling_options['Level3']['flag'] or modeling_options['Level2']['flag']:
            ...
            if modeling_options['WISDEM']['RotorSE']['flag']: 
                ...
                self.connect('xf.cl_interp_dac',             'stall_check_of.airfoils_cl')
                self.connect('xf.cd_interp_dac',             'stall_check_of.airfoils_cd')
                self.connect('xf.cm_interp_dac',             'stall_check_of.airfoils_cm')
            ...
            if modeling_options['ROSCO']['DAC_Mode']:
                # Connections to blade 
                self.connect('dac_ivc.dac_end',             'blade.outer_shape_bem.span_end')
                self.connect('dac_ivc.dac_ext',             'blade.outer_shape_bem.span_ext')
            ...
            if not modeling_options['Level3']['from_openfast']:
                ...
                self.connect('xf.cl_interp_dac',              'aeroelastic.airfoils_cl')
                self.connect('xf.cd_interp_dac',              'aeroelastic.airfoils_cd')
                self.connect('xf.cm_interp_dac',              'aeroelastic.airfoils_cm')
                ...
                self.connect('xf.dac_param',                  'aeroelastic.airfoils_Ctrl')
            ...
            if modeling_options['General']['openfast_configuration']['model_only'] == False:
                ...
                self.connect('tune_rosco_ivc.dac_kp_norm',       'outputs_2_screen_weis.dac_kp_norm')
                self.connect('tune_rosco_ivc.dac_tau',        'outputs_2_screen_weis.dac_tau')
                ...
                self.connect('dac_ivc.dac_end',            'outputs_2_screen_weis.dac_end')
