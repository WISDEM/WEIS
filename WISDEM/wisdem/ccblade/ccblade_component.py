...
class CCBladeTwist(ExplicitComponent):
    ...
    def compute(self, inputs, outputs, discrete_inputs, discrete_outputs):
        ...
        if self.options['opt_options']['optimization_variables']['blade']['aero_shape']['twist']['inverse']:
            ...
            for i in range(self.n_span):
                af[i].eval_unsteady(inputs['airfoils_aoa'], inputs['airfoils_cl'][i,:,0,0], inputs['airfoils_cd'][i,:,0,0], inputs['airfoils_cm'][i,:,0,0])
                alpha[i]    = (af[i].unsteady['alpha1'] - margin2stall) / 180. * np.pi
...
class AeroHubLoads(ExplicitComponent):
    ...
    def compute(self, inputs, outputs, discrete_inputs, discrete_outputs):
        ...
        ccblade = CCBlade(r, chord, theta, af, Rhub, Rtip, B,
            rho, mu, precone, tilt, yaw, shearExp, hub_height,
            nSector, precurve, precurveTip, tiploss=tiploss, hubloss=hubloss,
            wakerotation=wakerotation, usecd=usecd, derivatives=False)
...
class CCBladeEvaluate(ExplicitComponent):
    ...
    def setup(self):
        ...
        # outputs
        self.add_output('P',      val=0., units='W')
        self.add_output('T',     val=0., units='N')
        self.add_output('Q',     val=0., units='N/m')
        
        self.add_output('CP',      val=0.)
        self.add_output('CT',     val=0.)
        self.add_output('CQ',     val=0.)
