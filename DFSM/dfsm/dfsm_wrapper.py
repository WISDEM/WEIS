...
from dfsm.dfsm_utilities import valid_extension,calculate_time,valid_extension_DISCON,compile_dfsm_results
...
def dfsm_wrapper(fst_vt, modopt, inputs, discrete_inputs, FAST_runDirectory, FAST_namingOut):
    ...
    if usecase == 'test':
        ...
        if testing_data_availability == 'online':
            raise Exception('Testing data must be available offline for this usecase')
        ...
        # test dfsm
        dfsm,U_list,X_list,dx_list,Y_list = test_dfsm(dfsm,
                                                      test_cases = test_dataset,
                                                      test_ind = test_options['test_ind'],
                                                      simulation_flag = test_options['simulation_flag'],
                                                      plot_flag = test_options['plot_flag'],
                                                      solver_options = solver_options) 
        
        # outputs
        outputs_flag = (len(sim_details['reqd_outputs']) > 0)

        # get state and output properties
        state_props = model_options['state_props']
        output_props = model_options['output_props']
        ...
        # plot dfsm
        plot_dfsm_results(U_list,X_list,dx_list,Y_list,test_options['simulation_flag'],outputs_flag,save_flag = False)
        
        #calculate the time
        dfsm_time = calculate_time(dfsm)

        plt.show()

    elif usecase == 'simulation':
        ...
        for idx,ind in enumerate(test_ind):
            ...
            if testing_data_availability == 'online':
                ...
                wind_speed = test_data['controls'][:,0] # TODO: should be automated
                ...
                # start timer and solve for the states and controls
                t1 = timer.time()
                sol =  solve_ivp(run_sim_ROSCO,tspan,x0,method=solver_options['method'],args = (dfsm,param),rtol = solver_options['rtol'],atol = solver_options['atol'],t_eval = time_OF)
                t2 = timer.time()
                dfsm.simulation_time = (t2-t1)
