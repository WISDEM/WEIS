...
TYPE, PUBLIC :: ControlParameters
    ...
    Real(DbKi)                    :: dac_bb_depTime              ! Minimum amount of time a DAC device will deploy for when triggered (DAC_Mode = 4)
    INTEGER(IntKi)                :: dac_bb_useIDAC              ! A flag used to determine whether to use IDAC or general DAC in bang-bang DAC control mode (DAC_Mode =4)
...
TYPE, PUBLIC :: LocalVariables
    ...
    REAL(DbKi)                    :: dac_param(3)                ! DAC device parameter value (units of DAC device)
